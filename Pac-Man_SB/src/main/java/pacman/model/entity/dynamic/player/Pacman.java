...
public class Pacman implements Controllable, PlayerPositionSubject {
    ...
    public void collideWith(Level level, Renderable renderable) {
        if (level.isCollectable(renderable)) {
            ...
            if(collectable.isPowerPellet()){
                level.triggerFrightenedMode();
                ghostsEatenInFrightenedMode = 0;
            }
        } else if (renderable instanceof Ghost){
            Ghost ghost = (Ghost) renderable;
            
            switch (ghost.getGhostMode()) {
                case FRIGHTENED:
                    handleGhostEaten(level, ghost);
                    break;
                case RESPAWN:
                    break;
                default:
    ...
    public void handleGhostEaten(Level level, Ghost ghost){
        System.out.println("REspawn in scatter mode");
        ghost.respawnInScatterMode();
        ghostsEatenInFrightenedMode++;
        int points = 200 * (int)Math.pow(2, ghostsEatenInFrightenedMode - 1);
    ...
    public void registerObserver(PlayerPositionObserver observer) {
        ...
        observer.updatePlayerPosition(this.kinematicState.getPosition());
    }
    ...
    public void notifyObservers() {
        for (PlayerPositionObserver playerPositionObserver : observers) {
            playerPositionObserver.updatePlayerPosition(this.kinematicState.getPosition());
        }
    }
